var File, PLUGIN_NAME, ProgressBar, async, childProcess, electron, fs, getApmPath, grs, isFile, path, spawn, through2, util, wrench;
  ...
  async.eachSeries(options.platforms, function(platform, callback) {
    ...
    return async.series([
      function(next) {
        ...
        return next(null, platform.indexOf('darwin') < 0 && releasePath || path.join(releasePath, 'Electron.app'));
      }
      ...
      execution = (function() {
        switch (false) {
          case !(platform.indexOf('darwin') >= 0):
            return 'Contents/MacOS/Electron';
